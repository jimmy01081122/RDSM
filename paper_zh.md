...
預設 sample size 是 10,000。Full sampling 僅限 debug，且在 `duration_sec > 2` 或 `threads > 2` 時會被拒絕，除非明確 override。Canonical CLI mode 是 `bounded_rotation`；歷史使用的 `reservoir` 仍可作為 alias。此實作是 bounded rotating sample，不是 statistically uniform Algorithm R reservoir sampling。因此 final latency numbers 只能作為相同 collection policy 下的 prototype-relative tail indicators。
...
Historical matrix rows 已通過 stock/sold invariant checks。其 `duplicate_commit_count` 欄位為 0，但資料產生於 scoped detector 啟用之前，因此不能支持 historical no-duplicate-commit claim。修正後的 CTest 與 smoke runs 另行驗證 scoped same-`tx_id` duplicate-application detector。

### 10.2 Adaptive Routing Calibration

Calibration 使用 `routing_margin_us=5,10,20` 與 `cost_window_ms=100,250,500`，workloads 為 `low_uniform_read95`、`mixed_hot4_write50`、`high_hot16_write100`。54 historical rows 通過 stock/sold invariant checks，選定 default 為 `routing_margin_us=5`、`cost_window_ms=500`、`adaptive_object_scope=shard`、`hot_shards=8`。
...
Reduced focused final matrix 完成 324 synthetic rows：6 workloads、6 algorithms、threads 1/2/4、3 repetitions。所有 historical rows 通過 stock/sold invariant checks。
...
Reduced focused final matrix 完成 216 application-like rows：4 workloads、6 algorithms、threads 1/2/4、3 repetitions。所有 historical rows 通過 stock/sold invariant checks。
...
Controlled sold-counter comparison 完成 48 historical rows，位於 `results/final_sold_counter_comparison/`。設定包含 2 workloads、2 sold-counter modes、2 algorithms、threads 2/4、3 repetitions。所有 rows 通過 stock/sold invariant checks。
...
目前 phase-change historical artifacts 來自 multi-process approximation，會在不同 phase 間重啟 benchmark。它們可作 low-risk approximation，但不能證明 continuous in-process adaptive reaction。
...
另外，目前 `bounded_rotation` CLI mode（包含歷史 `reservoir` alias 的資料）是 bounded rotating sample，因此 tail-latency conclusion 只能被描述為 prototype-relative indicators。Counter schema version 2 使用 logical transaction 作為 abort-rate 分母；歷史 mixed-denominator rows 必須明確標記 schema-sensitive comparison。
